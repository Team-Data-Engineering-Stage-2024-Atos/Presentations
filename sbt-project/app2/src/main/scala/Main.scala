...
object Main {
  def main(args: Array[String]): Unit = {
    ...
    val config = ConfigManager.loadConfig(configFile).withValue("etl.output.target_table", ConfigValueFactory.fromAnyRef(outputTable))
    ...
    // val spark = SparkSession.builder().appName("app2").config("spark.master", "local[*]").getOrCreate()
