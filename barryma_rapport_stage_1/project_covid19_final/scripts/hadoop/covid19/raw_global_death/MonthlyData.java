...
public class MonthlyData {
    public static void main(String[] args) throws Exception {
        if (args.length != 2) {
            System.err.println("Usage: MonthlyData <input path> <output path>");
            System.exit(-1);
        }
        ...
        Job job = Job.getInstance(conf, "Monthly Data");
        job.setJarByClass(MonthlyData.class);
        job.setMapperClass(MonthlyMapper.class);
        job.setCombinerClass(MonthlyReducer.class);
