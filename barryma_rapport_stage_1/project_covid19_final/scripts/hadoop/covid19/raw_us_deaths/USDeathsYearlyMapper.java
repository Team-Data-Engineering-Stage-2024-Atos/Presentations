...
public class USDeathsYearlyMapper extends Mapper<LongWritable, Text, Text, LongWritable> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        Map<String, Long> yearlyDeaths = new HashMap<>();
        ...
        for (int i = 12; i < fields.length; i++) { // Skip to the 13th column (index 12)
            ...
            if (fields[i] != null && !fields[i].isEmpty()) {
                try {
                    ...
                    yearlyDeaths.put(year, yearlyDeaths.getOrDefault(year, 0L) + deathCount);
        ...
        for (Map.Entry<String, Long> entry : yearlyDeaths.entrySet()) {
            stateYear.set(state + ",Total_Deaths_Year_" + entry.getKey());
            deaths.set(entry.getValue());
            context.write(stateYear, deaths);
        }
    }
}
